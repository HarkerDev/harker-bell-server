...
router.get("/", async (req, res) => {
  res.redirect(`webcal://bell.harker.org/api/ical/feed?sv=1&t=${(new Date).getTime()}&source=root&excludedLunchDurations=45&includeSchedule&includeEvents=all`)
});

router.get("/feed", calendarHandler(false));
router.get("/download", calendarHandler(true));
  
function calendarHandler(isDownload) {
  return async (req, res) => {
    ...
    const customPeriodNames = req.body.periodNames || (req.query.periodNames && Buffer.from(req.query.periodNames, 'base64').toString().split(',')); // Custom period names
    ...
    try {
      await db.collection("schedules").find({
        ...
        // Add bell schedule
        if (Object.hasOwn(req.query, 'includeSchedule') || req.body.includeSchedule) {
          for (const block of document.schedule) {
            ...
            const periodNumber = block.name.match(/^P(\d)$/);
            const customPeriodName = periodNumber && customPeriodNames[periodNumber[1] - 1];
            ...
            events.push(setDefaultData({
              ...
              location: customPeriodName ? `P${periodNumber[1]}${block.location ? `: ${block.location}` : ``}` : block.location,
